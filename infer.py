...
samples_root 测试图像路径
ckpt_dir 模型权重路径
json_file_path GT的文件路径
'''
samples_root = './data/test_bbox'
json_file_path = './GT'
ckpt_dir = 'checkpoint/checkpoint-5510'
box_dict={}
json_data = []
cnt=0
list = []
json_list =[item for item in os.listdir(os.path.join(json_file_path, 'dataset')) if item.endswith('.json')]
with open(os.path.join(json_file_path, 'data_info.json'), 'r', encoding='utf-8') as f:
    data = json.load(f)
with open(os.path.join(json_file_path, 'desc_box.json'), 'r', encoding='utf-8') as f:
...
for item in data:
    ...
    CONCEPT = ''
    image_path = os.path.join(samples_root, word)+ '.png'
...
json_output_path = './poyi/inference_result.json'
