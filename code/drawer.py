...
import math
def draw_bboxes(image_path, bboxes, colors, output_path):
    """
    在图像上画出多个归一化的bbox，每个bbox使用不同的颜色。
    
    参数:
        image_path: 输入图像的路径。
        bboxes: bounding box 的列表，每个 bbox 格式为 [左上x, 左上y, 右下x, 右下y]，范围 [0, 1]。
        colors: 颜色列表，每个 bbox 对应一个颜色。
        output_path: 保存绘制结果的路径。
    """
    # 打开图像
    ...
    for i, bbox in enumerate(bboxes):
        ...
        # 获取对应的颜色，若颜色列表不够长则重复使用最后一个颜色
        color = colors[i] if i < len(colors) else colors[-1]

        # 画出矩形框
        draw.rectangle([left_x, top_y, right_x, bottom_y], outline=color, width=3)

    # 保存结果
    image.save(output_path)
    print(f"BBoxes 已绘制并保存到: {output_path}")
