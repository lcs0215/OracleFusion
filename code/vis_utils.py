...
def show_cross_attention(prompt: str,
    ...
    for i in range(len(tokens)):
        ...
        if i in indices_to_alter:
            heatmap, image = show_image_relevance(image, orig_image)
            image = image.astype(np.uint8)
            heatmap = heatmap.astype(np.uint8)
            
            image = np.array(Image.fromarray(image).resize((res ** 2, res ** 2)))
            heatmap = np.array(Image.fromarray(heatmap).resize((res ** 2, res ** 2)))
            
            image = ptp_utils.text_under_image(image, decoder(int(tokens[i])))
            # heatmap = ptp_utils.text_under_image(heatmap, decoder(int(tokens[i])))
            
            images.append(np.concatenate([image, heatmap], axis = 0))
            heatmaps.append(Image.fromarray(heatmap).resize((res ** 2, res ** 2)))

    return ptp_utils.view_images(np.stack(images, axis=0)), heatmaps
...
def show_image_relevance(image_relevance, image: Image.Image, relevnace_res=16):
    ...
    def show_heatmap_on_gray(img):
        viridis_colormap = plt.cm.get_cmap('viridis')
        viridis_image = viridis_colormap(img)
    ...
    # image_relevance = torch.softmax(image_relevance, dim = 0)
    ...
    # heatmap = cv2.applyColorMap(np.uint8(255 * image_relevance), cv2.COLORMAP_HOT)
    # image_relevance = torch.where(image_relevance > 0.3,
    #                               image_relevance, 0.3)
    heatmap = show_heatmap_on_gray(image_relevance)
