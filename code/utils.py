...
import save_svg
import cv2
from ttf import image_to_svgs, normalize_letter_size
import torch
import numpy as np
...
def preprocess(font, word, bbox=None, image_dir=None):
    ...
    bbox_scaled, obc_utils = image_to_svgs(image_dir, init_path, font_path, word, bbox)
    # normalize_letter_size(image_dir, init_path, font_path, word)


    print("Done preprocess")
    return bbox_scaled, obc_utils
...
def get_data_augs(cut_size):
    ...
    augmentations.append(K.Resize(size=(cut_size, cut_size), p=1.0))
...
def combine_word(word, letter, font, experiment_dir):
    word_svg_scaled = f"./code/data/init/{word}_scaled.svg"
    ...
    img = img[:, :, 3:4] * img[:, :, :3] + \
               torch.ones(img.shape[0], img.shape[1], 3, device="cuda:3") * (1 - img[:, :, 3:4])
    img = img[:, :, :3]
    save_image(img, f"{experiment_dir}/{word}_{letter}.png")
